...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Factory Operations', path: '/', icon: LayoutDashboard },
    { name: 'Device Registry', path: '/devices', icon: Cpu },
    { name: 'Edge Nodes', path: '/edge', icon: Workflow },
    { name: 'Real-time Telemetry', path: '/telemetry', icon: Activity },
    { name: 'Predictive Analytics', path: '/analytics', icon: Zap },
    { name: 'Sync status', path: '/sync', icon: CloudLightning },
    { name: 'OT/IT Security', path: '/security', icon: ShieldCheck },
    { name: 'Compliance Audit', path: '/audit', icon: Lock },
    { name: 'Digital Twin Hub', path: '/twin', icon: Boxes },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-emerald-400 bg-clip-text text-transparent">MFG.Landing</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Industrial Arch</p>
                <p className="text-xs text-slate-500 truncate">Factory Floor OT</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search devices, factory zones, edge nodes, firmware versions..." 
